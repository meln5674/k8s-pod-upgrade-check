...
// TODO: Make CreatedAt check optional and opt-in
// TODO: If pod image is date-formatted, or <date>.<version> formatted, check for tags with the same format,
//       and ignore such tags otherwise
...
func init() {
	...
	rootCmd.Flags().DurationVar(&cacheValidity, "cache-validity", 1*time.Hour, "Time to cache results of listing repository tags for.")
	rootCmd.Flags().StringArrayVar(&watchNamespaces, "watch-namespaces", nil, "Namespaces to watch. Omit to watch at cluster scope.")
}

// rootCmd represents the base command when called without any subcommands
var rootCmd = &cobra.Command{
	Use:   "k8s-pod-upgrade-check",
...
func runChecker(cmd *cobra.Command, args []string) error {
	...
	if len(watchNamespaces) == 0 {
		checker.watch(k8sinformers.NewSharedInformerFactory(k8sclient, syncInterval))
	} else {
		for _, ns := range watchNamespaces {
			checker.watch(k8sinformers.NewSharedInformerFactoryWithOptions(k8sclient, syncInterval, k8sinformers.WithNamespace(ns)))
		}
	}
...
func (c *Cache) GetTags(ctx context.Context, reg, repoName string, repo *ImageRepo, now time.Time, before time.Time) ([]ImageTag, error) {
	c.lock.Lock()
	defer c.lock.Unlock()

	if repo, ok := c.repos[repo.Named.Name()]; ok && repo.tagsAt.Before(before) {
		klog.V(3).Info("valid cached tags", "registry", reg, "repostiroy", repoName, "at", repo.tagsAt, "before", before)
		return repo.tagOrder, nil
	}
	...
	klog.V(3).InfoS("getting repo tags", "registry", reg, "repository", repoName)
	rawTags, err := docker.GetRepositoryTags(ctx, nil, repoRef)
	if err != nil {
		return nil, err
	}
	...
	cachedRepo := CachedRepo{
		tagOrder: tags,
		tagsAt:   now,
		tags:     make(map[string]*ImageTag, len(tags)),
	}
	for ix, tag := range cachedRepo.tagOrder {
		cachedRepo.tags[tag.Tag] = &cachedRepo.tagOrder[ix]
	}

	c.repos[repo.Named.Name()] = cachedRepo

	return tags, nil
}

func podImages(pod *corev1.Pod) map[string]map[string]*ImageRepo {
...
func newerSemver(ref ImageRef, tags []ImageTag) []ImageTag {
	newer := make([]ImageTag, 0, 5)
...
func (c *Checker) watch(factory k8sinformers.SharedInformerFactory) {
	factory.Core().V1().Pods().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc:    c.goCheckPodImages,
		UpdateFunc: c.goCheckPodImages2,
	})
	factory.Start(wait.NeverStop)
	factory.WaitForCacheSync(wait.NeverStop)
}

func (c *Checker) goCheckPodImages(obj any) {
	go c.checkPodImages(obj)
}

func (c *Checker) goCheckPodImages2(old, new any) {
	go c.checkPodImages(new)
}
func (c *Checker) checkPodImages(obj any) {
	now := time.Now()
	...
	if lastCheck, ok := c.skipPod(pod); ok {
	...
	for reg, repos := range repos {
		for repoName, repo := range repos {
			...
			go func(now time.Time, reg, repoName string, repo *ImageRepo, pod *corev1.Pod) {
				defer done.Done()
				c.checkPodRepo(now, reg, repoName, repo, pod)
			}(now, reg, repoName, repo, pod)
		}
	}
	done.Wait()
	c.setPodLastCheckTime(now, pod)
}

func (c *Checker) skipPod(pod *corev1.Pod) (lastCheck time.Time, skip bool) {
...
func (c *Checker) checkPodRepo(now time.Time, reg, repoName string, repo *ImageRepo, pod *corev1.Pod) {
	tags, err := c.cache.GetTags(c.ctx, reg, repoName, repo, now, now.Add(c.cacheValidity))
	if err != nil {
	...
	for _, ref := range repo.Tags {
		...
		c.upsertEvent(now, reg, repoName, ref, newer, pod)
	}
}

func (c *Checker) upsertEvent(now time.Time, reg, repoName string, ref ImageRef, newer []ImageTag, pod *corev1.Pod) {
	...
	eventName := fmt.Sprintf("%s.%s", pod.Name, hashHex)

	event, err := c.k8sclient.CoreV1().Events(pod.Namespace).Get(c.ctx, eventName, metav1.GetOptions{})
	if err == nil {
		// TODO: Check if this is actually our event? What happens otherwise?
		patch := corev1.Event{
			Message:       msg.String(),
		...
		if err != nil {
			klog.ErrorS(err, "failed to marshal event patch", "event", eventName)
			return
		}
		event.Message = msg.String()
		_, err = c.k8sclient.CoreV1().Events(pod.Namespace).Patch(c.ctx, eventName, types.StrategicMergePatchType, patchJSON, metav1.PatchOptions{})
		if err != nil {
			klog.ErrorS(err, "failed to patch event", "event", eventName)
		}
		klog.V(3).InfoS("patched event", "event", eventName)
		return
	}
	if !errors.IsNotFound(err) {
		klog.InfoS("failed to check for event, will create", "error", err)
	}
...
func (c *Checker) setPodLastCheckTime(now time.Time, pod *corev1.Pod) {
	patch := corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Annotations: map[string]string{
				lastCheckAnnotation: now.Format(time.RFC3339),
			},
		},
	...
	patchJSON, err := json.Marshal(&patch)
	if err != nil {
		klog.ErrorS(err, "failed to marshal pod patch")
	}
	...
	if err != nil {
		klog.ErrorS(err, "failed to patch pod with annotation", "json", string(patchJSON))
	}
}
